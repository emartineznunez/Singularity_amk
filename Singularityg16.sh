...
if G09DIR=$(dirname $(which g16 2> /dev/null) 2> /dev/null)
then
   if [ -d $GAUSS_SCRDIR ]
   then
      SINGULARITYENV_GAUSS_SCRDIR=/scratch SINGULARITYENV_PREPEND_PATH=/opt/g16 singularity instance start --bind $G09DIR:/opt/g16 --bind $GAUSS_SCRDIR:/scratch $siffile automekin$in
   else
      SINGULARITYENV_PREPEND_PATH=/opt/g16 singularity instance start --bind $G09DIR:/opt/g16 $siffile automekin$in
   fi
else
